...
class BitsAndBytesModelLoader(BaseModelLoader):
    ...
    def _unquantized_generator(
        ...
        for weight_name, weight_tensor in self._hf_weight_iter(
            ...
            if not matched and weight_name.endswith(".weight"):
                import sys
                print(f"[BNB-DBG] NO MATCH: {weight_name} shape={tuple(weight_tensor.shape)}", file=sys.stderr, flush=True)
            if matched:
                orig_weight_name = weight_name
                if "language_model" in weight_name:
                    orig_weight_name = orig_weight_name.replace(r"model.language_model.", r"model.")
                ...
                with set_default_torch_dtype(torch.float32):
                    ...
                    import sys
                    if weight_name.endswith("gate_proj.qweight") or "experts.0" in weight_name:
                        print(f"[BNB-DBG] QUANT {weight_name} in_shape={tuple(loaded_weight.shape)} out_shape={tuple(processed_weight.shape)} dtype={processed_weight.dtype} type={type(processed_weight).__name__}", file=sys.stderr, flush=True)

                quant_state_dict[orig_weight_name] = quant_state
                import sys
                if "shared_expert" in orig_weight_name or "mlp.gate" in orig_weight_name:
                    print(f"[BNB-DBG] STORED qstate: {orig_weight_name}", file=sys.stderr, flush=True)
            else:
    ...
    def _load_weights(self, model_config: ModelConfig, model: nn.Module) -> None:
        ...
        import sys
        print(f"[BNB-DBG] BEFORE: model={type(model).__name__} target_modules_count={len(self.target_modules)} first3={self.target_modules[:3]}", file=sys.stderr, flush=True)
        ...
        print(f"[BNB-DBG] AFTER: model={type(model).__name__} target_modules_count={len(self.target_modules)} first3={self.target_modules[:3]}", file=sys.stderr, flush=True)
        ...
        mlp_params = [n for n in param_dict if "shared_expert" in n and "layers.28" in n]
        mlp_gate_params = [n for n in param_dict if "layers.28" in n and ".gate." in n and "shared" not in n]
        print(f"[BNB-DBG] mlp_params 28: {mlp_params[:5]}", file=sys.stderr, flush=True)
        print(f"[BNB-DBG] mlp_gate_params 28: {mlp_gate_params[:5]}", file=sys.stderr, flush=True)
        ...
        for quant_param_name in quant_state_dict:
            ...
            if quant_param_name not in param_dict:
                import sys
                print(f"[BNB-DBG] POST SKIP: {quant_param_name}", file=sys.stderr, flush=True)
                continue
        ...
        for param_name, param in param_dict.items():
            if param_name in stacked_quant_state_dict:
                ...
                if pack_ratio == -1:
                    if "shared_expert" in param_name or "mlp.gate" in param_name or "mtp" in param_name:
                        continue
    ...
    def load_model(
        ...
        *,
        ...
        import sys
        print(f"[BNB-DBG] BitsAndBytesModelLoader.load_model ENTERED", file=sys.stderr, flush=True)
        quant_config = _get_quantization_config(model_config, self.load_config)
        print(f"[BNB-DBG] quant_config={quant_config}", file=sys.stderr, flush=True)
        with set_default_torch_dtype(model_config.dtype):
            with torch.device(device_config.device):
                model = _initialize_model(
                    model_config,
